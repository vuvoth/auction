...
contract NFTFactory {
	...
	function _createNFT(string memory _name, string memory _symbol, uint256 _totalSupply) private returns(address) {		 
		return address(new BurntNFT(admin, _name, _symbol, _totalSupply));
	}

	function createNFT(string memory _name, string memory _symbol, uint256 _totalSupply) external {
